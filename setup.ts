...
function newDay(dayStr: string) {
    if (validFile(`input/${dayStr}/input.txt`)) {
        console.log(red(`Day ${dayStr} already exists`));
        return;
    }
    ...
    const indexStr = (day: string) => {
        return `import part1 from "./part1.ts";\nimport part2 from "./part2.ts";\nimport * as utils from "../../utils/index.ts";\nconst data = utils.dayInputString('${day}');\nconst temp = globalThis.window.console.log\nif (false) globalThis.window.console.log = () => { }\n\nconst p1 = part1(data);\nconst p2 = part2(data);\n\nconsole.log("part1:", p1);\nconsole.log("part2:", p2);`;
    };

    Deno.mkdirSync(`input/${dayStr}`);
    Deno.mkdirSync(`days/${dayStr}`);
    Deno.writeTextFile(`input/${dayStr}/input.txt`, "");
    Deno.writeTextFile(`input/${dayStr}/test.txt`, "");
    Deno.writeTextFile(`days/${dayStr}/part1.ts`, part1Str);
    Deno.writeTextFile(`days/${dayStr}/part2.ts`, part2Str);
    Deno.writeTextFile(`days/${dayStr}/index.ts`, indexStr(dayStr));
}
...
if (!validFile(`input/${padNumber(day)}/test.txt`, true)) {
    ...
    if (!testInput) {
        ...
        const validInput = prompt(
            "Is this valid test input? (check screenshot) (y/n)",
        );
        if (validInput === "y") {
            Deno.writeTextFileSync(
                `input/${padNumber(day)}/test.txt`,
                testInput,
            );
        } else {
