export default function part1(data: string) {
  ...
  function nextRound() {
    for(let monkey of monkeys) {
      for(let item of monkey.items) {
        ...
        const newWorryValue = Math.floor(eval(`${Number(item)} ${op} ${opAmount}`));
        let throwTo;
        ...
        console.log(monkey.id, item, newWorryValue, throwTo.id, monkey.items.length, newWorryValue % Number(monkey.test))
      }
    }
  }
  ...
  console.log(monkeyBusiness, monkeys.map(m => ({ id: m.id, items: m.items, inspects: m.inspects})))
