...
export default function part1(data: string) {
  ...
  let distanceField = Array.from(
    { length: field.length },
    () => Array.from({ length: field[0].length }, () => -1),
  );
  ...
  while (!found && pipes.length > 0 && i < 10000) {
    let newPipes = [];
    for (let p of pipes) {
      if (i > 6000) console.log(pipes);
      const _p = explorePath(p[0], p[1], i);
      if (_p) newPipes.push(_p);
    }
    i++;
    pipes = newPipes.filter(Boolean).flat();
  }

  console.log("DONE!", pipes, start);
