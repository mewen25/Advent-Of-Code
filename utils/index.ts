...
export const dayInputString = (day: string, testData = false) =>
    Deno.readTextFileSync(
        `${__dirname}input/${day}/${testData ? "test" : "input"}.txt`,
    );
...
export const browserToQuestionPage = async (day: number, path?: string) => {
    ...
    // });
    await page.goto(path ?? `https://adventofcode.com/2024/day/${day}`);
...
export const fetchDayScreenshot = async (day: number) => {
    ...
    try {
        await page.screenshot({
            path: `${__dirname}days/${padNumber(day)}/question.png`,
            fullPage: true,
        });
    } catch (err) {
