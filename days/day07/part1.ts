...
export default function part1(data: string) {
  const hands = data.split("\n").map((h, hidx) => {
    ...
    // alpha sort
    const sortedCards = cards.split("").sort((a, b) => b.localeCompare(a))
    let countedCards = {}
    let check = null
    for (let i = 0; i < sortedCards.length; i++) {
      ...
      if (!check) { check = card }
      while (true) {
        if (sortedCards[i + j] === card) {
          j++
        }
        else break
      }
      if (hidx === 0) console.log([card, j])
      if (!countedCards[card]) countedCards[card] = j
      else countedCards[card] += j
      i += j - 1; continue
    }

    const values = Object.values(countedCards)
    // check five
  ...
  return rankOrder.reverse().reduce((acc, cur, idx) => {
    ...
    console.log(idx, '>', cur, rankNumber);
